...
class ZillaLikes {
	...
	function admin_init()
	{
		...
		add_settings_field( 'show_on', esc_html__( 'Automatically show likes on', 'zillalikes' ), array(&$this, 'setting_show_on'), 'zilla-likes', 'zilla-likes' );
		add_settings_field( 'exclude_from', esc_html__( 'Exclude from Post/Page ID', 'zillalikes' ), array(&$this, 'setting_exclude_from'), 'zilla-likes', 'zilla-likes' );
		add_settings_field( 'disable_css', esc_html__( 'Disable CSS', 'zillalikes' ), array(&$this, 'setting_disable_css'), 'zilla-likes', 'zilla-likes' );
		add_settings_field( 'ajax_likes', esc_html__('AJAX Like Counts', 'zillalikes'), array(&$this, 'setting_ajax_likes'), 'zilla-likes', 'zilla-likes');
		add_settings_field( 'zero_postfix', esc_html__( '0 Count Postfix', 'zillalikes' ), array(&$this, 'setting_zero_postfix'), 'zilla-likes', 'zilla-likes' );
		add_settings_field( 'one_postfix', esc_html__( '1 Count Postfix', 'zillalikes' ), array(&$this, 'setting_one_postfix'), 'zilla-likes', 'zilla-likes' );
		add_settings_field( 'more_postfix', esc_html__( 'More than 1 Count Postfix', 'zillalikes' ), array(&$this, 'setting_more_postfix'), 'zilla-likes', 'zilla-likes' );
		add_settings_field( 'instructions', esc_html__( 'Shortcode and Template Tag', 'zillalikes' ), array(&$this, 'setting_instructions'), 'zilla-likes', 'zilla-likes' );
	}
	
	function admin_menu() 
	{
		$icon_url = plugins_url( '/images/favicon.png', __FILE__ );
		$page_hook = add_menu_page( esc_html__( 'ZillaLikes Settings', 'zillalikes'), 'ZillaLikes', 'update_core', 'zilla-likes', array(&$this, 'settings_page'), $icon_url );
		add_submenu_page( 'zilla-likes', esc_html__( 'Settings', 'zillalikes' ), esc_html__( 'ZillaLikes Settings', 'zillalikes' ), 'update_core', 'zilla-likes', array(&$this, 'settings_page') );
		// ZillaFramework link
	...
	function setting_show_on()
	{
		...
		'. esc_html__('Posts', 'zillalikes') .'</label><br />
		...
		'. esc_html__('Pages', 'zillalikes') .'</label><br />
		...
		'. esc_html__('Blog Index Page, Archive Pages, and Search Results', 'zillalikes') .'</label><br />';
	}
	...
	function setting_exclude_from()
	{
		...
		<p class="description">'. esc_html__('Comma separated list of post/page ID\'s (e.g. 4,7,87)', 'zillalikes') . '</p>';
	}
	...
	function setting_disable_css()
	{
		...
		<label><input type="checkbox" name="zilla_likes_settings[disable_css]" value="1"'. (($options['disable_css']) ? ' checked="checked"' : '') .' />' . esc_html__('I want to use my own CSS styles', 'zillalikes') . '</label>';
		...
		if(strtolower($theme_name) == 'shutterbug'){
    		echo '<br /><span class="description" style="color:red">'. esc_html__('We recommend you check this option when using the Shutterbug theme to avoid conflicts', 'zillalikes') .'</span>';
		}
	}
	...
	function setting_ajax_likes()
	{
		...
		' . esc_html__('AJAX Like Counts on page load', 'zillalikes') . '</label><br />
		<span class="description">'. esc_html__('If you are using a cacheing plugin, you may want to dynamically load the like counts via AJAX.', 'zillalikes') .'</span>';
	}
	...
	function setting_zero_postfix()
	{
		...
		<span class="description">'. esc_html__('The text after the count when no one has liked a post/page. Leave blank for no text after the count.', 'zillalikes') .'</span>';
	}
	...
	function setting_one_postfix()
	{
		...
		<span class="description">'. esc_html__('The text after the count when one person has liked a post/page. Leave blank for no text after the count.', 'zillalikes') .'</span>';
	}
	...
	function setting_more_postfix()
	{
		...
		<span class="description">'. esc_html__('The text after the count when more than one person has liked a post/page. Leave blank for no text after the count.', 'zillalikes') .'</span>';
	}
	
	function setting_instructions()
	{
		echo '<p>'. esc_html__('To use ZillaLikes in your posts and pages you can use the shortcode:', 'zillalikes') .'</p>
		<p><code>[zilla_likes]</code></p>
		<p>'. esc_html__('To use ZillaLikes manually in your theme template use the following PHP code:', 'zillalikes') .'</p>
	...
	function do_likes()
	{
		...
		$output = $this->like_this($post->ID, $options['zero_postfix'], $options['one_postfix'], $options['more_postfix']);
  		...
  		$class = 'zilla-likes';
  		$title = esc_html__('Like this', 'zillalikes');
		if( isset($_COOKIE['zilla_likes_'. $post->ID]) ){
			$class = 'zilla-likes active';
			$title = esc_html__('You already like this', 'zillalikes');
		}
...
class ZillaLikes_Widget extends WP_Widget {
	...
	function __construct() {
		parent::__construct( 'zilla_likes_widget', 'ZillaLikes', array( 'description' => esc_html__('Displays your most popular posts sorted by most liked', 'zillalikes') ) );
	}
	...
	function form( $instance ) {
		...
		$defaults = array(
			'title' => esc_html__('Popular Posts', 'zillalikes'),
			'description' => '',
			'posts' => 5,
			'display_count' => 1
		);
